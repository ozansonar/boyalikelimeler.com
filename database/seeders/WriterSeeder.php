...
class WriterSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Create literary categories
        $categoryData = [
            ['name' => 'Şiir',               'slug' => 'siir',               'sort_order' => 1,  'is_active' => true],
            ['name' => 'Öykü',               'slug' => 'oyku',               'sort_order' => 2,  'is_active' => true],
            ['name' => 'Deneme',             'slug' => 'deneme',             'sort_order' => 3,  'is_active' => true],
            ['name' => 'Roman',              'slug' => 'roman',              'sort_order' => 4,  'is_active' => true],
            ['name' => 'Makale',             'slug' => 'makale',             'sort_order' => 5,  'is_active' => true],
            ['name' => 'Günlük',             'slug' => 'gunluk',             'sort_order' => 6,  'is_active' => true],
            ['name' => 'Gezi Yazısı',        'slug' => 'gezi-yazisi',        'sort_order' => 7,  'is_active' => true],
            ['name' => 'Kompozisyon',        'slug' => 'kompozisyon',        'sort_order' => 8,  'is_active' => true],
            ['name' => 'Biyografi',          'slug' => 'biyografi',          'sort_order' => 9,  'is_active' => true],
            ['name' => 'Eleştiri',           'slug' => 'elestiri',           'sort_order' => 10, 'is_active' => true],
            ['name' => 'Deyimler',           'slug' => 'deyimler',           'sort_order' => 11, 'is_active' => true],
            ['name' => 'Maniler',            'slug' => 'maniler',            'sort_order' => 12, 'is_active' => true],
            ['name' => 'Kişisel Gelişim',    'slug' => 'kisisel-gelisim',    'sort_order' => 13, 'is_active' => true],
            ['name' => 'Astroloji',          'slug' => 'astroloji',          'sort_order' => 14, 'is_active' => true],
            ['name' => 'Quizler',            'slug' => 'quizler',            'sort_order' => 15, 'is_active' => true],
            ['name' => 'Şarkı Sözleri',     'slug' => 'sarki-sozleri',      'sort_order' => 16, 'is_active' => true],
            ['name' => 'Araştırma',          'slug' => 'arastirma',          'sort_order' => 17, 'is_active' => true],
            ['name' => 'Kitaplar',           'slug' => 'kitaplar',           'sort_order' => 18, 'is_active' => true],
            ['name' => 'Eğitim',             'slug' => 'egitim',             'sort_order' => 19, 'is_active' => true],
            ['name' => 'Gündem',             'slug' => 'gundem',             'sort_order' => 20, 'is_active' => true],
            ['name' => 'Çocuk Edebiyatı',    'slug' => 'cocuk-edebiyati',    'sort_order' => 21, 'is_active' => true],
            ['name' => 'Kültür',             'slug' => 'kultur',             'sort_order' => 22, 'is_active' => true],
            ['name' => 'Tarih',              'slug' => 'tarih',              'sort_order' => 23, 'is_active' => true],
            ['name' => 'Sanat',              'slug' => 'sanat',              'sort_order' => 24, 'is_active' => true],
            ['name' => 'Videolu Gönderiler', 'slug' => 'videolu-gonderiler', 'sort_order' => 25, 'is_active' => true],
            ['name' => 'Fotoğraflı Gönderiler', 'slug' => 'fotografli-gonderiler', 'sort_order' => 26, 'is_active' => true],
        ];

        foreach ($categoryData as $cat) {
            LiteraryCategory::updateOrCreate(
                ['slug' => $cat['slug']],
                $cat,
            );
        }

        $categories = LiteraryCategory::pluck('id', 'slug');
        ...
        $allWorks = $this->getWorksData();
        $adminUser = User::whereHas('role', fn ($q) => $q->whereIn('slug', ['admin', 'super_admin']))->first();
        ...
        foreach ($writers as $index => $writerData) {
            ...
            $writerWorks = $allWorks[$index];

            foreach ($writerWorks as $workData) {
                $categoryId = $categories[$workData['category_slug']] ?? $categories->first();
                $hasRevision = $workData['has_revision'] ?? false;
                $revisionReason = $workData['revision_reason'] ?? null;
                unset($workData['category_slug'], $workData['has_revision'], $workData['revision_reason']);

                $work = LiteraryWork::updateOrCreate(
                    ['slug' => $workData['slug']],
                    array_merge($workData, [
                        'literary_category_id' => $categoryId,
                        'user_id'              => $user->id,
                        'view_count'           => rand(30, 800),
                        'published_at'         => $workData['status'] === LiteraryWorkStatus::Approved
                            ? now()->subDays(rand(1, 90))
                            : null,
                    ]),
                );

                // Create revision record for revision_requested works
                if ($hasRevision && $adminUser && $revisionReason) {
                    LiteraryRevision::updateOrCreate(
                        ['literary_work_id' => $work->id, 'admin_id' => $adminUser->id],
                        [
        ...
        return [
            ...
            [
                ...
                [
                    ...
                    'has_revision'     => true,
                    'revision_reason'  => 'Şiirin ikinci kıtası biraz daha detaylandırılabilir. Ayrıca son kıtadaki geçiş çok ani, araya bir kıta daha ekleyerek akışı yumuşatabilir misin?',
                ],
            ],
            ...
            [
                ...
                [
                    ...
                    'has_revision'     => true,
                    'revision_reason'  => 'Deneme çok güzel başlamış ama sonuç paragrafı biraz zayıf kalmış. E-kitap ile basılı kitap karşılaştırmasını daha dengeli yapabilir misin? Ayrıca bilimsel referansın kaynağını da belirtirsen daha iyi olur.',
                ],
            ],
