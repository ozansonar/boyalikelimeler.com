...
@section('title', 'Edebiyat Kategorileri — Admin')

@section('content')

    <x-admin.page-header title="Edebiyat Kategorileri" subtitle="Edebiyat eserlerinin kategorilerini yönetin">
        <a href="{{ route('admin.literary-categories.create') }}" class="btn-teal">
    ...
    <div class="card-dark mb-4" data-aos="fade-up" data-aos-delay="100">
        <div class="card-body-custom">
            <form method="GET" action="{{ route('admin.literary-categories.index') }}" class="cl-toolbar">
                <div class="cl-search">
                ...
                <div class="cl-toolbar-actions">
                    ...
                    @if(!empty($filters['search']) || isset($filters['is_active']) && $filters['is_active'] !== '')
                        <a href="{{ route('admin.literary-categories.index') }}" class="btn-glass"><i class="bi bi-x-lg me-1"></i>Temizle</a>
                    @endif
                </div>
            </form>
        </div>
                    ...
                    <tbody>
                        @forelse($categories as $category)
                            <tr>
                                ...
                                <td>
                                    <span class="cl-category-badge tech">{{ $category->works_count }}</span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="usr-actions">
                                        <a class="usr-action-btn" title="Düzenle" href="{{ route('admin.literary-categories.edit', $category) }}">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <button class="usr-action-btn danger" title="Sil" onclick="openDeleteModal({{ $category->id }}, '{{ addslashes($category->name) }}', {{ $category->works_count }})">
                        ...
                        @empty
                            <x-admin.table-empty :colspan="7" icon="bi-folder2-open" message="Henüz edebiyat kategorisi oluşturulmamış." />
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-body text-center py-4">
                    ...
                    <p class="cl-modal-warning d-none" id="deleteWarning"><i class="bi bi-exclamation-circle me-1"></i>Bu kategoride eserler bulunuyor, silinemez.</p>
...
@push('scripts')
<script>
function openDeleteModal(id, name, workCount) {
    document.getElementById('deleteItemName').textContent = name;
    document.getElementById('deleteForm').action = '/admin/literary-categories/' + id;
