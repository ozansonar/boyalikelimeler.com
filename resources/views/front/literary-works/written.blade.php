...
@section('title', 'Yazılı Eserler — Boyalı Kelimeler')
@section('meta_description', 'Boyalı Kelimeler yazarlarının kaleme aldığı en güzel yazılı eserler. Şiir, hikaye, deneme, roman ve daha fazlası.')
@section('canonical', route('literary-works.written'))
@section('og_title', 'Yazılı Eserler — Boyalı Kelimeler')
@section('og_description', 'Yazarlarımızın kaleme aldığı en güzel yazılı eserler.')
...
{!! json_encode([
    ...
    '@graph' => [
        [
            '@type' => 'CollectionPage',
            'name' => 'Yazılı Eserler',
            'description' => 'Boyalı Kelimeler yazarlarının kaleme aldığı en güzel yazılı eserler.',
            'url' => route('literary-works.written'),
            'isPartOf' => [
                '@type' => 'WebSite',
        ...
        [
            ...
            'itemListElement' => [
                ...
                ['@type' => 'ListItem', 'position' => 2, 'name' => 'Yazılı Eserler'],
            ],
        ],
    ],
...
@section('content')
    ...
    <!-- Page Hero -->
    <section class="clist-hero" aria-label="Yazılı Eserler başlığı">
        <div class="container">
            <div class="clist-hero__inner">
                <h1 class="clist-hero__title">
                    <i class="fa-solid fa-feather-pointed me-3"></i>Yazılı Eserler
                </h1>
                <p class="clist-hero__desc">
                    Yazarlarımızın kaleminden dökülen en güzel yazılı eserler. Şiirden hikayeye, denemeden romana — kelimelerin boyandığı dünya.
                </p>
    ...
    <section class="clist-section" aria-label="Yazılı eser listesi">
        <div class="container">

            <!-- Toolbar: Search + Filter + Grid Switcher -->
            <form class="clist-toolbar" method="GET" action="{{ route('literary-works.written') }}" id="clistFilterForm">
                <div class="clist-toolbar__search">
                    ...
                    <input type="text"
                           ...
                           placeholder="Yazılı eserlerde ara..."
                           value="{{ request('ara') }}">
                </div>
            ...
            <div class="clist-grid" id="clistGrid" data-cols="3">
                ...
                @empty
                    <div class="clist-empty">
                        ...
                        <h3 class="clist-empty__title">Yazılı Eser Bulunamadı</h3>
                        <p class="clist-empty__desc">
                            @if(request('ara'))
                                <strong>"{{ request('ara') }}"</strong> araması için sonuç bulunamadı.
                            @else
                                Henüz yazılı eser bulunmamaktadır.
                            @endif
                        </p>
                        <a href="{{ route('literary-works.written') }}" class="clist-empty__btn">
                            <i class="fa-solid fa-arrows-rotate me-2"></i>Tüm Yazılı Eserleri Gör
                        </a>
                    </div>
                @endforelse
