...
@section('title', ($pageSettings['golden_brush_title'] ?? 'Altın Fırçalarımız') . ' — Boyalı Kelimeler')
@section('meta_description', ($pageSettings['golden_brush_description'] ?? 'Boyalı Kelimeler altın fırça ressamları. Her ay en iyi ressamlarımızı seçiyoruz.'))
@section('canonical', route('painters.golden-brush-index'))
@section('og_title', ($pageSettings['golden_brush_title'] ?? 'Altın Fırçalarımız') . ' — Boyalı Kelimeler')
@section('og_description', ($pageSettings['golden_brush_description'] ?? 'Boyalı Kelimeler altın fırça ressamları. Her ay en iyi ressamlarımızı seçiyoruz.'))
...
{!! json_encode([
    ...
    '@graph' => [
        [
            '@type' => 'CollectionPage',
            'name' => ($pageSettings['golden_brush_title'] ?? 'Altın Fırçalarımız') . ' — Boyalı Kelimeler',
            'description' => $pageSettings['golden_brush_description'] ?? 'Boyalı Kelimeler altın fırça ressamları.',
            'url' => route('painters.golden-brush-index'),
        ],
        [
            '@type' => 'BreadcrumbList',
            'name' => 'Breadcrumb',
            'itemListElement' => [
                ...
                ['@type' => 'ListItem', 'position' => 2, 'name' => $pageSettings['golden_brush_title'] ?? 'Altın Fırçalarımız'],
            ],
        ],
    ],
            ...
            <div class="page-header__inner">
                ...
                @if(!empty($pageSettings['golden_brush_description']))
                    <p class="page-header__desc">
                        {{ $pageSettings['golden_brush_description'] }}
                    </p>
                @endif
            </div>
        </div>
            ...
            <nav aria-label="Breadcrumb">
                <ol class="golden-breadcrumb">
                    ...
                    <li class="golden-breadcrumb__item golden-breadcrumb__item--active">
                        {{ $pageSettings['golden_brush_title'] ?? 'Altın Fırçalarımız' }}
                    </li>
                </ol>
            </nav>
        </div>
